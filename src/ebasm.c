...
static BmInst bm_translate_line(StringView line) {
  line = sv_ltrim(line);
  ...
  return (BmInst)BM_INST_NOP;
}

static size_t bm_translate_source(StringView source, BmInst *program,
                                  size_t program_cap) {
...
int main(int argc, char *argv[]) {
  ...
  bm.program_size = bm_translate_source(sv, bm.program, BM_PROGRAM_CAP);

  if (!bm_save_program_to_file(bm.program, bm.program_size, output_file))
    return EXIT_FAILURE;
